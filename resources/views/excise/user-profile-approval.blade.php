...
<main role="main" class="col-md-9 ml-sm-auto col-lg-10 pt-3 px-4">
    ...
    <h1 class="h2">Users Profile Approvals</h1>    
    </div>
    ...
    <table class="table table-striped table-sm">
        ...
        <tr>
            ...
            <td><a href="/{{$user->user_id}}">{{ $user->fetchforadmin->cnic }}</td>
            <td>{{ $user->fetchforadmin->email }}</td>
            <td><a href="/{{$user->id}}">{{ $user->first_name.' '.$user->last_name }}</a></td>
            <td class="d-flex">
                <form action="{{ route('admin.user-profile-approval-submit')}}" method="POST" class="mr-1">
                @csrf
                @method('POST') 
                ...
                <form action="{{ route('admin.user-profile-approval-submit')}}" method="POST">
                @csrf
                @method('POST') 
